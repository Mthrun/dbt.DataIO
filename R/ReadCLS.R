...
# author: Alfred Ultsch             
  ...
  if (prod(is.na(Z[DataInd,2]))==1) {Cls = as.data.frame(Z[DataInd,1]) # 2nd col == NA -> read only first column
		 ...
		 if(sum(!is.finite(suppressWarnings(as.numeric(ClsKey))))==0)
			mode(ClsKey) = 'numeric'
  }
  colnames(Cls) = 'Cls'
 ...
 # }
  setwd(CurrentDir)
  ClassColors=NULL
  return(list(ClsKey=ClsKey, Cls=as.vector(Cls), UniqueClasses = UniqueClasses, ClassColors = ClassColors,Comments=Comments))
