ReadSparseMatrix <- function(FileNameWithoutExt,InDirectory=getwd()){
  ...
  if(any(Files==paste0(FileNameWithoutExt,'.names'))){
  	...
  	if(dim(NamesRaw$FurtherTexts)[2] == 1){# Adj-Matrix mit rownames = colnames
  	  GeneDesc = NamesRaw$FurtherTexts[,1]
  		DimNames <- list('rownames'=NamesRaw$Names,'colnames'=NamesRaw$Names)
  	}else{ # This case should NOT occur!!!
  	  GeneDesc = NamesRaw$FurtherTexts[,1]
  		DimNames <- list('rownames'=NamesRaw$Names[NamesRaw$FurtherTexts[,2]=='rowname'],
  		                 'colnames'=NamesRaw$Names[NamesRaw$FurtherTexts[,2]=='colname'])
  ...
  return(list("ZeilenInd"  = ZeilenInd,
              "SpaltenInd" = SpaltenInd,
              "Inhalt"     = Inhalt,
              "Dimension"  = Dimension,
              "Header"     = Header,
              "Key"        = Key,
