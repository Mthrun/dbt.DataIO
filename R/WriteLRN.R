WriteLRN = function(FileName, Data, Header, Key=c(), DataDefined=c(), OutDirectory=getwd(),  CommentOrDigits=NULL, Digits = NULL, scientific = F)
...
#       }
      ...
      # use colnames of matrix if available
      ...
      # change all keys above 10^10 into scientific notation
        upperLimitUntilSci = 10^10
      if(any(DataFin[,1] > upperLimitUntilSci)){
        warning("Some of the provided Keys are above the Limit of 10^10 and will be changed into Scientific Notation which may introduce non unique keys.")
        DataFin[,1] = sapply(DataFin[,1], function(k) ifelse(k>upperLimitUntilSci, format(k, scientific = T, digits = 3), format(k, scientific = F, digits = 0)))
      }
      
      # change entries to scientific notation
      if(!is.null(Digits))
        DataFin = cbind(DataFin[,1], format(DataFin[,2:ncol(DataFin)], scientific = scientific, digits = Digits))
